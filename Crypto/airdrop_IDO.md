### Plexus
* 现在swap一次只有6points，0.06U， 还不值手续费，不建议做，但是这个crew3可以做
* https://medium.com/@plexus.app/plexus-galxe-passport-oat-giveaway-463224a7f91d


### whitehole finance
这是arbitrum的 借贷平台 收益率挺高，   

https://galxe.com/whiteholefinance/campaign/GCGS9UfcvD
https://whitehole.finance/

The $GRV IDO will take place on 📆April 26
About IDO: 
WL price : 1 $GRV = $0.015 
Public price : 1 $GRV = $0.02 
Listing Price : 1 $GRV = $0.03

Starting April 26th 11:00 am UTC and will last until April 28th 11:30 am UTC.
You can contribute with USDC in exchange for GRV tokens
100,000,000 GRV(out of a 1,000,000,000 total supply) will be allocated to the sale.
When each round of hardcap($450,000/$1,400,000) is reached, IDO will be closed.
Claiming Process

Users who click "Claim" will immediately receive 50% of the reserved GRV tokens.
If they choose to "Lock" their tokens, the remaining 50% will be immediately 
locked in the GRV Vault for a duration of one month, or for the duration 
previously specified when the tokens were locked in the GRV Vault.
Alternatively, users can choose to "Claim (After 1 month)" to
receive the remaining tokens after the one-month period has elapsed.

e1,e2,  deposit 10 U，得到NFT， 但是white list价格一下就秒了
e2，只有在0.02的价格买了100U，
```
The $GRV IDO will take place on 📆April 26
About IDO: 
WL price : 1 $GRV = $0.015 
Public price : 1 $GRV = $0.02 
Listing Price : 1 $GRV = $0.03
```

```
KAVA, VARA
Home1
KAVA 原来是在aptos上， 用kplr钱包，后面支持evm，出现了kava evm
地址，Kava链上的AMM， 存了20U，领了10个VARA空投	
https://app.kava.io/evm/wkava	
https://equilibrefinance.com/liquidity	
```
```
𝗘𝗰𝗵𝗼𝗗𝗘𝗫

@Echo_DEX
 is the first DEX on Linea #Consensy zkEVM testnet.
 
https://chainlist.org/chain/59140
https://www.testnet.echodex.io/faucet
```


### SEI
https://testnet.sparrowswap.xyz/
https://www.seinetwork.io/treasure
https://usdcfaucet.sparrowswapxyz.com/  
https://app.seinetwork.io/faucet/  

task
https://edge.blocked.cc/products/1


### Kepler
```bash
kepler，得到5个mystery box，还没开
https://arb-claim.kepler.homes/
```

### AIDOGE 
```bash
aidoge 已经登陆OKX交易所
空投得到的aidoge， 在uniswap添加流动性
```
### karate-combat
钱包需要500U以上才能保证参加IDO， 等5月2号以后
https://daomaker.com/company-sale-rounds/karate-combat/public/Dn3tR6bwvw
